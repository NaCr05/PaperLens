...
test("keeps local PDF reading, direct Codex calls, scrolling, zoom, and mobile controls wired", async () => {
  ...
  assert.match(page, /previousTask\.cancel\(\)/);
  ...
  assert.match(page, /accumulatePageTurnIntent/);
  assert.match(page, /onWheel=\{handleStageWheel\}/);
  assert.match(page, /onTouchMove=\{handleStageTouchMove\}/);
  assert.match(page, /PAGE_TURN_COOLDOWN_MS/);
  assert.match(page, /result\.turn === 1 \? "top" : "bottom"/);
  assert.match(page, /适合宽度（100%）/);
  assert.match(page, /buildPageSegments/);
  assert.match(page, /const gutterThreshold = Math\.max\(viewport\.width \* \.045/);
